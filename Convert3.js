...
(async () => {
    try {
        ...
        // Read file names from the source folder
        const fileNames = fs.readdirSync(sourceFolderPath);

        // Filter file names based on image formats
        const imageFileNames = fileNames.filter(file => {
            return ['.png', '.jpg', '.jpeg'].includes(path.extname(file).toLowerCase());
        });

        // Process files in batches
        for (let i = 0; i < imageFileNames.length; i += batchSize) {
            const batchFiles = imageFileNames.slice(i, i + batchSize);
            const batchPaths = batchFiles.map(file => path.join(sourceFolderPath, file));

            console.log(`Processing batch ${i / batchSize + 1}:`, batchPaths);

            try {
                const files = await imagemin(batchPaths, options);

                console.log(`Conversion successful for batch: ${i / batchSize + 1}`);
                files.forEach(file => {
                    // Log the file object to inspect its structure
                    console.log(`File processed: `, file);
                    // Manually write files to the destination folder
                ...
                console.error(`Conversion failed for batch: ${i / batchSize + 1}`, batchError);
            }
        }
    } catch (error) {
